...
order by f1.addr_state,year(f5.last_credit_pull_d);
	...
	# Home ownership Vs last payment date stats
     with e as (select last_pymnt_d, home_ownership, last_pymnt_amnt from finance_2_upd
	JOIN finance_1
	ON finance_1.id = finance_2_upd.id) SELECT
  e.home_ownership, max_date,
  format(round(sum(last_pymnt_amnt),2),0) AS Latest_Payment
  FROM
  e
  JOIN (
    ...
    SELECT 
      home_ownership,
      MAX(last_pymnt_d) AS max_date
    FROM e
    GROUP BY home_ownership
  ) name_dates
    -- Join table to subquery on both name and the aggregate date
    ON e.home_ownership = name_dates.home_ownership AND e.last_pymnt_d = max_date
-- Group for the outer SUM()
GROUP BY e.home_ownership, max_date
order by e.home_ownership;
